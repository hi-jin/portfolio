...
export default function ContentSection({ content }: { content: Array<{ id: string; date: string; title: string; thumbnail: string; category: string }> }) {
  return (
        ...
        <ul className="mb-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {content.map((item) => (
            <li key={item.id} className="mb-2">
              <Link href={`/category/${item.category}`} className="text-blue-500 hover:underline block p-4 bg-gray-100 dark:bg-gray-800 rounded-lg transition-colors duration-200 hover:bg-gray-200 dark:hover:bg-gray-700">
